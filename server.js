...
app.post('/ask', async (req, res) => {
    ...
    try {
        ...
        const systemPrompt = `Bạn là trợ lý AI thông minh về lịch sử Đại tướng Võ Nguyên Giáp và Quân đội Nhân dân Việt Nam. 
        ...
        res.status(500).json({ reply: "Lỗi kết nối máy chủ AI.", suggestions: ["Chiến thắng Điện Biên Phủ?", "Tư tưởng quân sự của Đại tướng?"] }); 
    }
});
...
app.post('/whatif', async (req, res) => {
    ...
    try {
        ...
        const systemPrompt = `Bạn là Cỗ máy Đa Vũ Trụ lịch sử Việt Nam thế kỷ 20, tập trung vào Đại tướng Võ Nguyên Giáp.
        ${langInstruction}
        ...
        if (history.length === 0) {
            try {
                const imageResponse = await openai.images.generate({ model: "dall-e-2", prompt: `Tranh lịch sử Việt Nam thời kháng chiến u ám: ${scenario}`, n: 1, size: "512x512" });
...
app.post('/roleplay', async (req, res) => {
    ...
    try {
        ...
        const systemPrompt = `Bạn là Quản trò Game Nhập vai Lịch sử. Người chơi là Đại tướng Võ Nguyên Giáp. Lượt hiện tại: ${turn} / 10.
        ...
        res.status(500).json({ error: true, npcDialogue: "Lỗi kết nối." }); 
    }
});

// ==========================================
// 4. ÂM THANH & LƯU BÁO CÁO
// ==========================================
...
app.post('/save-report', async (req, res) => {
    ...
    // Link Google Sheet dùng chung cho cả Lê Hoàn, Ngô Quyền và Võ Nguyên Giáp
...
app.listen(port, () => { console.log(`✅ Server Backend Võ Nguyên Giáp chạy tại http://localhost:${port}`); });